...
async def make_changes_command(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, 'Новые картинки?', reply_markup=admin_kb.button_case_admin)
    await message.delete()

#@dp.message_handler(commands='Загрузить', state=None) #начало диалога загрузки пункта меню
async def cm_start(message : types.Message):
    if message.from_user.id == ID:
        await FSMAdmin.photo.set()
        await message.reply('Загрузить фото')

#@dp.message_hundler(state="*", commands='отмена')
